...
public class AthenaMod {
    ...
    public void onRightClickBlock(PlayerInteractEvent.RightClickBlock evt) {
        ...
        // Debug for forge energy stuff
        else if (evt.getWorld().getTileEntity(evt.getPos()) != null) {
            TileEntity te = evt.getWorld().getTileEntity(evt.getPos());
            te.getCapability(CapabilityEnergy.ENERGY).ifPresent(otherEnergy -> {
                if(!evt.getWorld().isRemote){
                    evt.getPlayer().sendMessage(new StringTextComponent("Energy: " + otherEnergy.getEnergyStored() + "/" + otherEnergy.getMaxEnergyStored()));
                }
                // send message on client and server
//                evt.getPlayer().sendMessage(new StringTextComponent("Energy: " + otherEnergy.getEnergyStored() + "/" + otherEnergy.getMaxEnergyStored()));
            });
        }
    }
